...
Image_show::Image_show(wxString sciezka, wxImage &obraz, wxString window_name, char metoda, wxWindow* parent,wxWindowID id,const wxPoint& pos,const wxSize& size)
{
    ...
    //*)
    SetTitle(window_name); //nazwa okna
    /*if (obraz2!=NULL)
    {
    ...
    wxString temp;
    ntp_gettime(&start);
    Obraz_bmp->SetBitmap(obraz); //miniaturka przed

    obrazek=new IMG(sciezka);//Oryginalny obraz
    ...
    //==================================================== tu metody
    if(metoda==1)
        obrazek_po=obrazek->Negacja();
    else if(metoda==2)
        obrazek_po=obrazek->Progowanie();
    else if(metoda==3)
    ...
    if (obrazek_po->GetWidth()>250 || obrazek_po->GetHeight()>250)
    {
        ...
        Obraz_po->SetBitmap(scaled_po); //scale() skaowanie!!!!
    }
    else
    {
